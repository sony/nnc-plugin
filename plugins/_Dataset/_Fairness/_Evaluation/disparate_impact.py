...
def disparate_impact(unprivileged, privileged, unprivileged_weights, privileged_weights):
    """
...
def func(args):
    ...
    di = disparate_impact(out_unprivileged, out_privileged, unprivileged_weights,
                          privileged_weights)
...
def main():
    ...
    parser.add_argument(
        ...
        help='specify the reweighing weights that are assigned to individual samples.'
             'If not provided, then each sample is given a unit weight, default=NA',
        default='NA'
    )
    parser.add_argument(
        '-n', '--num_samples',
