...
def plot_fairness(model_fairness, args,
    ...
    Args:
        ...
        ideal_fairness (list) : model ideal fairness.
    ...
    if ideal_fairness[0] <= model_fairness <= ideal_fairness[1]:
        ...
        ax.text(0, model_fairness + 0.1,
                ...
                bbox=dict(facecolor='red', alpha=0.3))
        ax.text(1.11, 1, "Fair", fontweight='bold', color='darkgrey')
        fig.text(0.92, 0.6, '\n'.join(
            [f"\nFairness :\n{metric} :  {model_fairness:.3f}"]), fontsize='15')
        if model_fairness < 1:
            fig.text(0.92, 0.55, '\n'.join(
...
def disparate_impact(output_unprivileged, output_privileged, classification_threshold):
    """
...
def func(args):
    ...
    di = disparate_impact(out_unprivileged, out_privileged, args.clf_threshold)
...
def main():
    ...
    parser.add_argument(
        '-i', '--input',
        help='path to input csv file, output result shown '
             'in the evaluation tab (csv) default=output_result.csv',
