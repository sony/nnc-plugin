...
def plot_fairness(model_fairness, args,
                  ...
                  metric="Equal Opportunity", file_path=None):
    """
...
def equal_opportunity(y_privileged, y_unprivileged, preds_privileged, preds_unprivileged, classification_threshold):
    """
    ...
    logger.log(99, 'Equal opportunity difference: %.4f' % (eod.item()))
    return round(eod.item(), 4)
...
def func(args):
    ...
    if not fair_flag:
        logger.error(f'Fairness value out of range! '
                     f'Fairness of this is metric between -1.0 and 1.0 , Please enter again', exc_info=0)
        sys.exit(0)
    else:
        logger.log(99,
    ...
    eod = equal_opportunity(y_privileged, y_unprivileged,
                            preds_privileged, preds_unprivileged, args.clf_threshold)
    plot_fairness(eod, args, metric=metric,
                  ideal_fairness=ideal_fairness, file_path=file_path)
    with open(args.output, 'w', newline="\n", encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Fairness Plot', 'Equal Opportunity'])
        writer.writerow([file_path, eod])
...
def main():
    parser = argparse.ArgumentParser(
        description='Equal Opportunity\n'
                    ...
                    'This metric is computed as the difference between true positive rate (true positives / positives) '
                    'between the unprivileged and the privileged groups. \n' +
    ...
    parser.add_argument(
        '-o', '--output', help='path to output file (csv) default=equal_opportunity.csv',
        required=True, default='equal_opportunity.csv')
