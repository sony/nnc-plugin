...
class LabelShuffleCsvDataSource(CsvDataSource):
    ...
    def _add_shuffled_index(self):
        for position, (row, shuffled_index) in enumerate(zip(self._rows, self._order)):
    ...
    def _add_shuffled_label(self, label_shuffle):
        ...
        if _shuffled_label_column not in self._variables_dict.keys():
            if label_shuffle:
                ...
                labels = np.array(self._get_labels()).astype(
                    int).reshape(-1, 1)
    ...
    def save_to_csv(self, filename, header=None, data_type=str, convert_to_abs_path=True):
        if header is None:
