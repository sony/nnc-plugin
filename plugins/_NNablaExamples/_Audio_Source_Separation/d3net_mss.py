...
def func(args):
    ...
    model_dir = os.path.join(get_data_home(), 'd3net-mss')
    shutil.unpack_archive(os.path.join(
        get_data_home(), os.path.basename(model_url)), model_dir)
    ...
    code = os.path.join(os.path.dirname(__file__), '..', 'ai-research-code',
                        'd3net', 'music-source-separation', 'separate.py')
    python_path = os.path.dirname(code)
    sys.path.append(python_path)
    os.chdir(python_path)
    command = ['python', code, '-i', input_file,
               '-m', model_dir, '-o', output_abs_dir]
    ...
    for track in ['bass', 'drums', 'vocals', 'other']:
        filename = os.path.join('.', os.path.basename(
            output_dir), input_base_name, f'{track}.wav')
...
def main():
    parser = argparse.ArgumentParser(
        description='D3Net Music Source Separation\n\n' +
        'Densely connected multidilated convolutional networks for dense prediction tasks\n' +
        'Naoya Takahashi, Yuki Mitsufuji\n' +
        'https://arxiv.org/abs/2011.11844\n\n',
    ...
    parser.add_argument(
        '-o',
        '--output',
        help='path to output csv file(csv), default=d3net_output.csv',
        required=True)
