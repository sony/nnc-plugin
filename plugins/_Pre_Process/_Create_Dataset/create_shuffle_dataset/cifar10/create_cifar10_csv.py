...
def df_to_csv(base_path, csv_file_name, data_path, df):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), unit='images'):
        relative_path = '/'.join((data_path, labels[row.label], f'{idx}.png'))
...
def main(args):
    ...
    train_csv_df = df_to_csv(path, 'cifar10_training.csv', './training', _df)

    if args.label_shuffle:
        logger.log(99, 'Creating "cifar10_training_shuffle.csv"... ')
        ...
        put_csv(os.path.join(path, 'cifar10_training_shuffle.csv'),
                train_csv_df)
