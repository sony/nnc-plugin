...
def func(args):
    args.max_words = args.max_charactors
    create_dataset(args, split_string)


def main():
    parser = argparse.ArgumentParser(
        description='String Classification\n\n' +
        'Convert a string classification dataset consisting of string x and label y to NNC format dataset.\n'
        'Convert a string x to a character index series.\n\n',
        formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument(
        '-i',
        '--input',
        help='string classification dataset consisting of string x and label y (csv)',
        required=True)
    parser.add_argument(
        '-E',
        '--encoding',
    ...
    parser.add_argument(
        '-l',
        '--max-length',
        help='maximum string length by the number of charactors (int) default=64',
        required=True,
        type=int,
        default=64)
    parser.add_argument(
        '-w',
        '--max-charactors',
        help='maximum number of charactors (int) default=256',
        required=True,
        type=int,
        default=256)
    parser.add_argument(
        '-m',
        '--min-occurences',
        help='minimum number of charactor occurrences (int) default=10',
        type=int,
        default=10)
    parser.add_argument(
        '-n',
        '--normalize',
        help='normalize characters with unicodedata (bool) default=False',
        action='store_true')
    parser.add_argument(
