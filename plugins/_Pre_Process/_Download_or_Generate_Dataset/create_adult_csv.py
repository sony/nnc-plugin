...
def func(args):
    ...
    for subset in ['data', 'test']:
        ...
        command = [sys.executable, os.path.join(os.path.abspath(os.path.dirname(__file__)), '..', '_Create_Dataset', 'simple_tabular_data.py'), '-i', os.path.join(path, csv_file_name), '-b', 'label', '-t', '-{}'.format(
...
def main():
    ...
    parser.add_argument(
        '-o',
        '--output-dir',
        help='path to write NNC dataset CSV format (dir) default=Structured\\Classification\\adult',
        required=True)
